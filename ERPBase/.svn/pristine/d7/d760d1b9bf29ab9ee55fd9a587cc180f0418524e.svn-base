...
initFileupload = function () {
    $('#fileupload').fileupload('option', {
        url: '../Account/UploadPhotoProfile',
        maxFileSize: 5000000,
...
function loadimage(reload) {
	...
	if (userid) {
		...
		if (reload)
		{
			window.location.href = "/Account/EditPhoto?userid="  + userid + "&t=" + d.getTime();
			return
		}


$("#imgTarget").attr("src", "../Account/userphoto?userid=" + userid + "&t=" + d.getTime());
	}
};
...
var navigateToDetailPage = function () {
    window.location = "/Account/Profile/userid=" + userid;
