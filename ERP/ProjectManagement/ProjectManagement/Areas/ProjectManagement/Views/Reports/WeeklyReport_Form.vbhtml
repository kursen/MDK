@ModelType ProjectManagement.WeeklyReport 
<style type="text/css">
    tr.tbHead {
...
<div class="row" id="form_Weekly"">
    <div class="col-sm-12">
    @Using Html.BeginForm("SaveWeeklyReport", "Reports", FormMethod.Post, New With {.autocomplete = "Off", .class = "form-horizontal", .id = "frmWeekly"})
        @Html.Hidden("ID")
        @Html.Hidden("ProjectInfoId")
        @Html.Hidden("WeekNumber")
        
        @Html.WriteFormInput(Html.TextBox("MingguKe", Nothing, New With {.class = "form-control", .disabled = "disabled"}), "Minggu Ke-", smControlWidth:=5, lgControlWidth:=3)
        ...
        @Html.WriteFormInput(Html.TextBox("Place", Nothing, New With {.class = "form-control"}), "Lokasi Laporan", smControlWidth:=5, lgControlWidth:=3)
        ...
        $('#btnSavePrint').click(function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.stat == 1) {
                        showNotification("Data telah berhasil disimpan");
                        window.location.href = "/ProjectManagement/Reports/WeeklyReportPrint?id=" + data.WkID + "&forMC0=@IIf(ViewData("ForMC0") = False OrElse IsNothing(ViewData("ForMC0")), "", "true")";
                        return false;
                    }
                    showNotificationSaveError(data);
                },
