@code
    ...
    ViewData("ForMC0") = True
End Code
@Html.Partial("ProjectPageMenu", ViewData("ProjectInfo"))


@Using Html.BeginJUIBox("Laporan Mingguan MC0")
        ...
        <div id="layerRptForm" style="display:none;">
            ...
            <div class="row">
                <div class="col-md-12 col-xs-12">
                    @Html.Partial("WeeklyReport_Form", ViewData("WeeklyReport"))
                </div>
            </div>
        </div>
        ...
        <div id="layerList">
            <div class="row">
                <div class="col-sm-12 col-lg-12">
                    <table id="tb_WeekList" class="table table-striped table-hover table-heading table-datatable dataTable responsive no-footer scrollTable" width="100%">
...
<script type="text/javascript">
    ...
    GetList = function () {
        ...
        attr.usingId.tableId = '#tb_WeekList';
        attr.url = {
            "Read": "ProjectManagement/Reports/GetReportListWeekItems/" + @model.Id,
            "Delete": "ProjectManagement/Reports/RemoveWeeklyDetail"
        };
        attr.dataTable.columns = [
            ...
            },
            {
                ...
                "mRender":function(data, type, full){
                   return full.WeekNumber + " ("  + formatShortDate(full.BeginDate) + " s/d " + formatShortDate(full.EndDate) + ")";
                }
            },
            {
                "data": "ID",
        ...
        $(attr.usingId.tableId + ' tbody')
            .on('click', '.print', function () {
                ...
                window.location.href = "/ProjectManagement/Reports/WeeklyReportPrint?Id=" + Data.ID + "&forMC0=" + true;
            })
...
<script type="text/javascript">
    ...
    _openDetail = function (Data) {
        ...
        $("#layerList").fadeOut(400, function () {
            $("#layerRptForm").show("slide", { direction: "right" }, 430);
            $("#MingguKe").val(Data.WeekNumber + " (" + formatShortDate(Data.BeginDate) + " s/d " + formatShortDate(Data.EndDate) + ")");
            $("#ProjectInfoId").val(@Model.Id);
            $("#WeekNumber").val(Data.WeekNumber);
    ...
    $(function () {
	   ...
	   $(window).bind('scroll', function() {
           ...
           var objTHead = $('#fixedHeader');
           var objTBody = $('bodyContent');
	        if ($(window).scrollTop() > navHeight) {
		        $(objTHead).addClass('fixed-header');
		        $(objTHead).find('tr th:nth-child(0)').width($(objTBody).find('tr:nth-child(0) td:nth-child(0)').width());
                console.log($(objTBody).find('tr:nth-child(0) td:nth-child(0)').html());
	        }
	        else {
