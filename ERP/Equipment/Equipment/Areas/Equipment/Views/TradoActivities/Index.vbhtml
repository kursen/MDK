@Code
    ViewData("Title") = "Daftar Kegiatan Trado"
...
<div class="row">
    <div class="col-xs-12">
        <div class="pull-right">
            <a href="@Url.Action("RptEmptyTrado", "Report")" class="btn btn-sm btn-primary  btn-add btn-label-left" >
                <span><i class="fa fa-cloud-download"></i></span>Download Formulir Laporan</a>
               @If isProperUser Then
            @<a href="@Url.Action("Create", "TradoActivities")" class="btn btn-sm btn-primary  btn-add btn-label-left" >
...
@code
    Using Html.BeginJUIBox("Kegiatan Trado", True, False, False, False, False, "fa fa-truck")
...
<script type="text/javascript">
    ...
    var _localLoad = function (data, callback, setting) {
        ...
        $.ajax({
            url: '/TradoActivities/getListActivity',
    ...
    $(document).ready(function () {
        ...
        .on("click", ".detail", function (d) {
            ...
            window.location.href = '/Equipment/TradoActivities/Detail/' + dataItem.ID;
        })
        .on("click", ".edit", function () {
            var tr = $(this).closest('tr');
            var row = tblactivity.row(tr);
            var dataItem = row.data();
            window.location.href = '/Equipment/TradoActivities/Edit/' + dataItem.ID;
        })
         .on('click', '.create', function () {
             ...
             window.location.href = '/Equipment/TradoActivities/Create/?id=' + dataItem.TRID + "&reportdate=" + reportdate.format("MM-DD-YYYY");
         });
