@modelType  Equipment.VehicleMaintenanceCheck
@Code
    ViewData("Title") = "Skema Perawatan"
...
@Functions
    ...
    Function WriteSpanControlKM(ByVal value As String, ByVal id As String) As MvcHtmlString
        Return New MvcHtmlString("<p class='form-control-static'>" & value & "</p>")
    End Function
    ...
    End Function
End Functions
@Using Html.BeginJUIBox("Skema Perawatan Kendaraan")
    
    Using Html.BeginForm("SaveVehicleMaintenance", "VehiclePeriodicMaintenance", Nothing, FormMethod.Post, New With {.class = "form-horizontal", .id = "frmvehiclemaintenance", .autocomplete = "off"})
    
    @<div class="row">
        <div class="col-sm-12 col-lg-12 form-horizontal">
            @*note: somehow, using html.hidden() doesn't work well here. *@
            ...
            @Html.WriteFormInput(WriteSpanControl(Model.Vehicle.Merk & "/" & Model.Vehicle.Type, "spMerkType"), "Merk/Type")
            @Html.WriteFormInput(WriteKmControl(Model.AverageUsagePerKilometer, "AverageUsagePerKilometer"), "Pemakaian Harian", lgControlWidth:=3)
        </div>
    </div>
        End Using
    @<div class="row">
        <div class="col-sm-12 col-lg-12">
            ...
            <div id="form-item" style="display: none;">
                ...
                @Using Html.BeginForm("ValidateVehicleCheckItem", "VehiclePeriodicMaintenance", Nothing, FormMethod.Post, New With {.class = "form-horizontal", .id = "frmitem", .autocomplete = "off"})
            Using Html.BeginJUIBox("Kegiatan Perawatan", False, False, False, False, False, "fa fa-wrench")
                    @Html.HiddenFor(Function(m) m.ID, New With {.id = "IDVehicleMaintenanceItem"})
                    @Html.Hidden("VehicleMaintenanceCheckId", 0)
                    ...
                    @<div class="row">
                        ...
                        @Html.WriteFormInput(WriteKmControlKM(0, "LastCheck_Kilometer"), "KM Terakhir",
                                     smLabelWidth:=4, smControlWidth:=4, lgLabelWidth:=3, lgControlWidth:=2)
                        @Html.WriteFormInput(Html.DateInput("LastCheck_Date", Nothing, dateFormat), "Tanggal Check KM Terakhir",
                                     smLabelWidth:=4, smControlWidth:=4, lgLabelWidth:=3, lgControlWidth:=2)
                        @*  @Html.WriteFormInput(WriteKmControlKM("LastCheck_Kilometer", "spKilometer"), "Pemakaian Harian", lgControlWidth:=3)*@
                        <div class="col-sm-12 col-lg-12">
                            <div class="well">
            ...
            <div class="well">
                <div style="width: 200px" class='center-block'>
                    <button type="button" class="btn btn-primary" id="btn-save">
                        Simpan</button>
                    <a href='@Url.Action("Index", "VehiclePeriodicMaintenance")' class="btn btn-primary">
                        Kembali</a>
                </div>
            </div>
        </div>
...
<script type="text/javascript">
    ...
    submitFormCallback = function (data) {
        if (data.stat == 0) {
            ...
            window.location = "/Equipment/VehiclePeriodicMaintenance/Index/";
        }
    }
    ...
    initTblSchema = function () {
        ...
        var _localLoad = function (data, callback, setting) {
            ...
            $.ajax({
                url: '/VehiclePeriodicMaintenance/GetListMaintenanceSchemaItem',
        ...
        var arrColumns = [
              ...
              {"data": "LastCheck_Kilometer", "sClass": "text-right" }, //
              {"data": "VehicleMaintenanceCheckId", "sClass": "text-center", "mRender": _renderEditColumn }
        ];
...
//        datatableDefaultOptions.order = [[2, "asc"]];
        ...
        .on("click", ".delete", function (d) {
            ...
            $.ajax({
                type: 'POST',
                url: '/VehiclePeriodicMaintenance/DeleteMaintenanceItem',
                data: { id: row.data().ID },
                dataType: 'json',
        ...
        .on("click", ".edit", function () {
            ...
            $("#IDVehicleMaintenanceItem").val(dataItem.ID);
            $("#rowIdx_item").val(row.index());

            $("#VehicleMaintenanceCheckId").val(dataItem.VehicleMaintenanceCheckId);
            $('#ItemName').val(dataItem.ItemName);
            $('#UnitValue').val(dataItem.UnitValue);
            $('#Value').val(dataItem.Value);
            $('#LastCheck_Kilometer').val(dataItem.LastCheck_Kilometer);
    ...
    $(function () {
        ...
        $('#btnadditem').click(function () {
            ...
            $('#IDVehicleMaintenanceItem').val(0)
            $('#VehicleMaintenanceCheckId').val(0)
            $('#rowIdx_item').val(-1);
        ...
        $('#btn-saveItem').click(function () {
            ...
            //console.log($('#LastCheck_Date').val());
            $.ajax({
                type: 'POST',
                data: _data,
                url: _url,
                success: function (data) {
                    if (data.stat == 1) {
                        var NumberId = $('#IDVehicleMaintenanceItem').val();
        ...
        $('#btn-save').click(function (e) {
            ...
            var _data = $('#frmvehiclemaintenance').serializeArray();
            var _urlaction = $('#frmvehiclemaintenance').attr('action');
            $.ajax({
                type: 'POST',
                data: _data,
                url: _urlaction,
