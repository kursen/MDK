...
{
    public class ReductionCorrectionController : BaseController
    {
        //
        // GET: /SKA/ReductionCorrection/

        private SKAEntities entities;
        private string reductionCorrectionSessionName = "ReductionCorrectionSession";

        public ReductionCorrectionController()
        {
        ...
        public ActionResult GetList() 
        {
            var model = from c in entities.CorrectionandBackups
                        where c.Name == "Koreksi Pengurangan"
        ...
        {
            Session.Remove(reductionCorrectionSessionName);

            var model = new AdditionalCorrectionViewModel();
            model.Name = "Koreksi Pengurangan";
        ...
        {
            var reductionCorrectionDetailSessionlist = (List<AdditionalCorrectionDetailViewModel>)(Session[reductionCorrectionSessionName]);
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    var newReductionCorrection = new CorrectionandBackup();
                    var existItem = entities.CorrectionandBackups.Where(a => a.Year == model.Year && a.Name != "Koreksi Penambahan" && a.Name != "Cadangan").FirstOrDefault();

                    if (existItem == null)
                    {
                        newReductionCorrection.Name = model.Name;
                        newReductionCorrection.Description = model.Description;
                        newReductionCorrection.Year = model.Year;

                        entities.CorrectionandBackups.AddObject(newReductionCorrection);

                        foreach (var item in reductionCorrectionDetailSessionlist)
                        {
                            var detail = new CorrectionandBackupDetail
                            {
                                CorrectionandBackupId = newReductionCorrection.Id,
                ...
                {
                    Logger.Error("Error while executing ReductionCorrectionController.Create[Post].", ex);
        ...
        {
            var model = (List<AdditionalCorrectionDetailViewModel>)Session[reductionCorrectionSessionName];
            ...
            if (model == null)
            {
                ...
                Session[reductionCorrectionSessionName] = model;
            }
        ...
        {
            IList<AdditionalCorrectionDetailViewModel> model = (IList<AdditionalCorrectionDetailViewModel>)Session[reductionCorrectionSessionName];
        ...
        {
            var additionalCorrectionDetailSessionlist = (List<AdditionalCorrectionDetailViewModel>)(Session[reductionCorrectionSessionName]);
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    Logger.Error("Error while executing ReductionCorrectionController.Edit [Post].", ex);
        ...
        public ActionResult Delete(int id)
        {
            ...
            {
                Logger.Error("Error while executing ReductionCorrectionController.Delete [Get].", ex);
                return RedirectToAction("Index");
            }
        }
        ...
        public ActionResult Delete(AdditionalCorrectionViewModel model)
        {
            ...
            {
                Logger.Error("Error while executing ReductionCorrectionController.Delete [Post].", ex);
