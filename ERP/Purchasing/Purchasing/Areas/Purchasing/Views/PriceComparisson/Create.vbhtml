...
@Using Html.BeginJUIBox("Daftar Permintaan Pembelian")
    ...
    autocomplete="off">
    @Html.Hidden("idOffice", 0, New With {.id = "spOfficeId"})
     @Html.Hidden("norecordId", 0, New With {.id = "spIDNoRecord"})
...
<script type="text/javascript">
    ...
    var _initTblItem = function () {
        ...
        var arrColumnsOp = [
            ...
            { 'data': 'TotalEstPrice', 'sClass': 'text-right t1' },
             { 'data': 'EstUnitPrice', 'sClass': 'text-right', 'mRender': _renderpopovercost },
    ...
    $(document).ready(function () {
        ...
        if ($('#RequestNumber').val() == null || $('#RequestNumber').val() == '') {
            _initautocompletenoRecord();
        }
        else {
            $('#RequestNumber').attr('readonly', true);
            tablecomparison.ajax.reload();
        }
