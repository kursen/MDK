...
@Code
    ...
    Dim dt As Date = Model.RequestDate
    Dim minDate As Date = New Date(dt.Year, dt.Month, dt.Day)
    Dim dateFormat = New With {.format = "dd-mm-yyyy", .autoclose = "true",
                                               .orientation = "auto", .todayBtn = "linked", .language = "id", .startDate = "d"}
    
    If Model.DocApproveRejectDate Is Nothing Then
        Model.DocApproveRejectDate = Date.Now
    End If
End Code
@Functions
    ...
    <div class="col-lg-12 col-sm-12">
        <div class="pull-right">
            <div class="button-group">
                <a href="@Url.Action("Index", "DepartmentPurchasingRequestApproval")" class="btn btn-danger btn-label-left">
                    <span><i class="fa fa-arrow-left"></i></span>Kembali</a> <a href="@Url.Action("PrintPR", "DepartmentPurchaseRequisition", New With {.id = Model.ID})" class="btn btn-danger btn-label-left">
    ...
    @<div class="row">
        <div class="col-sm-12 col-lg-12">
            <div class="form-horizontal" id="documentView">
                <input type="hidden" id="documentid" value ="@Model.Id" />
                @Html.WriteFormInput(WriteSpanControl(Model.RequestDate.ToString("dd-MM-yyyy"), "spRequestDate"), "Tanggal Dokumen")
                @Html.WriteFormInput(WriteSpanControl(Model.RecordNo, "spRecordNo"), "No. Dokumen")
                @Html.WriteFormInput(WriteSpanControl(Model.RequestType.Name, "spRequestType"), "Tipe Permintaan")
    ...
    $(function () {
        ...
        $("#btnDeleteDocument").click(function () {
            ...
            if (confirm("Hapus dokumen ini ?")) {
                $.ajax({
                    ...
                    url: "/Purchasing/DeparmentPurchasingRequest/DeleteDocument",
                    datatype: 'json',
                    success: function (data) {
                        if (data.stat == 1) {
                            window.location = "/Purchasing/DepartmentPurchasingRequestApproval/Index";
                        }
                    }
                });
            }
        ...
        $("#UpdateState").click(function () {
            ...
            $.ajax({
                ...
                url: "/Purchasing/DepartmentPurchasingRequestApproval/UpdateDocState",
                datatype: 'json',
                success: function (data) {
                    if (data.stat == 1) {
                        window.location = "/Purchasing/DepartmentPurchasingRequestApproval/Index";
                    } else {
                        showNotificationSaveError(data, "Update Status gagal.");
                    }
                }
            });
